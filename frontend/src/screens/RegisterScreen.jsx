...
// ✅ Physical device → use your PC IP: 192.168.1.11
const BASE_URL = Platform.OS === 'android'
  ? 'http://10.0.2.2:8000'    // Android emulator magic IP
...
const authService = {
  ...
  // POST /auth/register
  // ✅ Returns { access_token, token_type, user } — no separate login needed
      ...
      <View key={i} style={[styles.stepWrapper, i < totalSteps - 1 && { flex: 1 }]}>
        ...
        {i < totalSteps - 1 && (
          <View style={styles.stepLineContainer}>
            ...
            <View style={[styles.stepLineFill, { width: i < currentStep ? '100%' : '0%' }]} />
          </View>
        )}
      </View>
    ))}
...
const RegisterScreen = ({ navigation }) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // ✅ Single call — register returns { access_token, token_type, user }
      const data = await authService.register(payload);

      // ✅ Save to AsyncStorage for use across the app
      ...
      // ✅ Handle both string and array FastAPI error formats
...
const styles = StyleSheet.create({
  ...
  // Header
  header:    { marginTop: Platform.OS === 'ios' ? 52 : 24, marginBottom: 20 },
  appName:   { fontSize: 28, fontWeight: '800', color: DARK, letterSpacing: -0.5 },
  ...
  stepDotDone:       { backgroundColor: PURPLE },
  stepDotInner:      { width: 8, height: 8, borderRadius: 4, backgroundColor: '#fff' },
  stepCheck:         { color: '#fff', fontSize: 12, fontWeight: '800' },
  stepLineContainer: {
    flex: 1, height: 3, backgroundColor: '#E0E0F0',
    marginHorizontal: 6, borderRadius: 2, overflow: 'hidden',
  },
  stepLine:     { ...StyleSheet.absoluteFillObject, backgroundColor: '#E0E0F0' },
  stepLineFill: { height: '100%', backgroundColor: PURPLE, borderRadius: 2 },
  ...
  stepSubtitle: { fontSize: 13, color: GREY, marginBottom: 24, lineHeight: 18 },
  ...
  // Chips
  fieldLabel:       { fontSize: 13, fontWeight: '600', color: '#3D3D5C', marginTop: 16, marginBottom: 10 },
  chipRow:          { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  chip:             { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, backgroundColor: '#F0F0FA', borderWidth: 1.5, borderColor: '#E0E0F0', marginBottom: 4 },
  chipSelected:     { backgroundColor: PURPLE_LIGHT, borderColor: PURPLE },
  chipText:         { fontSize: 13, color: GREY, fontWeight: '500' },
  chipTextSelected: { color: PURPLE, fontWeight: '700' },
  ...
  outlineBtnText: { color: PURPLE, fontSize: 15, fontWeight: '600' },
  btnDisabled:    { opacity: 0.6 },
  navRow:         { flexDirection: 'row', gap: 12, alignItems: 'flex-end' },
