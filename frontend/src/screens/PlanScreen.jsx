  ...
  <View style={stepSt.wrapper}>
    {Array.from({ length: TOTAL_STEPS }).map((_, i) => (
      <React.Fragment key={i}>
        <View style={[stepSt.dot, i === currentStep && stepSt.dotActive, i < currentStep && stepSt.dotDone]}>
          {i < currentStep
...
const Chip = ({ label, emoji, selected, onPress }) => (
  <TouchableOpacity
    style={[chipSt.chip, selected && chipSt.chipSel]}
    onPress={onPress}
    activeOpacity={0.75}
  >
...
const ToggleRow = ({ options, selected, onSelect, formatter }) => (
  <View style={togSt.row}>
    {options.map(opt => (
      <TouchableOpacity
        key={opt}
        style={[togSt.pill, selected === opt && togSt.pillSel]}
...
const CityCard = ({ city, selected, onPress }) => (
  <TouchableOpacity
    style={[citySt.card, selected && citySt.cardSel]}
    onPress={onPress}
    activeOpacity={0.8}
  >
...
const PlanScreen = ({ navigation }) => {
  ...
  const [selectedCity, setSelectedCity] = useState(null); // full city object with lat/lng
  ...
  const handleNext = () => {
    ...
    // ✅ Final step — send lat/lng of selected city to backend
    const payload = {
      user_id:         user?.id,
      latitude:        selectedCity.latitude,   // ✅ actual lat from city object
      longitude:       selectedCity.longitude,  // ✅ actual lng from city object
      ...
      <View style={s.chipWrap}>
        {COMPANION_TYPES.map(c => (
          <Chip
            ...
            label={c.label}
            emoji={c.emoji}
            selected={companion === c.key}
            onPress={() => setCompanion(c.key)}
          />
        ))}
      </View>
    </>
  ...
  const Step2 = (
    <>
      ...
      <View style={s.summaryCard}>
        ...
        {[
          {
            icon: selectedCity?.emoji || '📍',
            label: selectedCity?.label || 'Not set',
            sub: selectedCity
          ...
          {
            icon: '👥',
            label: companion ?? 'Not specified',
            sub: '',
          },
          {
            icon: '📏',
            label: `Within ${distance} km  ·  ${limit} recommendations`,
            sub: '',
          },
        ].map((row, i) => (
  ...
  return (
    <View style={s.screen}>
      ...
      >
        ...
        <View style={s.btnRow}>
          {step > 0 && (
            <Button title="Back" onPress={handleBack} variant="outline" style={s.btnBack} />
          )}
          <Button
            title={isLast ? 'Get Recommendations →' : 'Continue →'}
            onPress={handleNext}
